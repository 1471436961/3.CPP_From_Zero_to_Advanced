/*************************************************************************
	> File Name: 14.priority_queue.cpp
	> Author: zhouyingda
	> Mail: 
	> Created Time: Mon 23 Mar 2026 10:39:40 AM CST
 ************************************************************************/
...
private:
    ...
    void expand() {
        ...
        this->swap(q);
        return ;
    } // q 为局部变量，expand 退出时会自动调用 q 的析构函数
    // STL 中的 queue 没有 top(), 故设为私有的
...
private:
    ...
    int *raw_data, *data; 
    // priority_queue 底层为堆，使用偏移技巧方便计算孩子节点
    int count, __size;
    CMP_T cmp;
    void expand() {
    ...
    // STL 中的 priority_queue 没有 front(), 故设为私有的
    int  front() const override { return data[1]; } 

public:
    priority_queue(CMP_T cmp = less<int>(), int n = 10)
...
int main() {
    int op, val;
    test::queue q1;
    test::priority_queue q2;
    test::priority_queue q3(cmp);
    while (cin >> op) {
        switch (op) {
            case 0: {
                cin >> val;
